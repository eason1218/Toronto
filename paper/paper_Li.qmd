...
#| warning: false
#| message: false
# Convert OCCUPANCY_RATE_BEDS to numeric (in case of any issues during import)
analysis_data$OCCUPANCY_RATE_BEDS <- as.numeric(analysis_data$OCCUPANCY_RATE_BEDS)

# Calculate the average occupancy rate per location
avg_occupancy_rate <- aggregate(OCCUPANCY_RATE_BEDS ~ LOCATION_NAME, analysis_data, mean, na.rm = TRUE)

# Sort the data by occupancy rate
avg_occupancy_rate <- avg_occupancy_rate[order(avg_occupancy_rate$OCCUPANCY_RATE_BEDS), ]

# Plot the bar chart with smaller text
ggplot(avg_occupancy_rate, aes(x = reorder(LOCATION_NAME, OCCUPANCY_RATE_BEDS), y = OCCUPANCY_RATE_BEDS)) +
  geom_bar(stat = "identity", fill = "skyblue", width = 0.6) +  # Use a more pleasing color
  coord_flip() +  # Flip to make it horizontal
  labs(title = "", x = "Location", y = "Occupancy Rate (%)") +
  theme_minimal() +  # Clean theme for better aesthetics
  theme(
    plot.title = element_text(size = 10, face = "bold", hjust = 0.5),  # Smaller title size
    axis.title = element_text(size = 9),  # Smaller axis title size
    axis.text.x = element_text(size = 7),  # Smaller size of x-axis text
    axis.text.y = element_text(size = 3.8),  # Smaller size of y-axis text (locations)
    panel.grid.major.y = element_blank()  # Remove y-axis grid lines for better readability
  ) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100))  # Ensure bars stay within 0-100% limits
```
...
@fig-occupancy-capital-relat displays the average occupancy rate across various shelters. The X-axis represents the occupancy rate percentage (Occupancy Rate %), ranging from 0% to 100%, while the Y-axis lists the shelter locations (Location), showing the names of several shelters. Each horizontal blue bar corresponds to the average occupancy rate of a specific shelter. Most shelters have an occupancy rate close to 100%, indicating that their beds are almost fully or completely occupied. A few shelters have occupancy rates below 75%, showing that they have relatively lower bed utilization. 
...
@fig-occupancy-capital-relat paints a sobering picture of Toronto's shelter system, with most facilities operating at or near full capacity. This level of demand points to a systemic issue that extends beyond shelter availability to broader socioeconomic challenges. The high occupancy rates reflect not just a lack of shelters but underline key drivers of homelessness, such as inadequate affordable housing, mental health challenges, unemployment, and other economic pressures.
